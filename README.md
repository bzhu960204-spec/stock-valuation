...
| **EV/FCF** | **东方财富** | **`RPT_F10_FINANCE_MAINFINADATA` (FCFF_BACK, INTEREST_DEBT_RATIO) + `RPT_DMSK_FN_BALANCE` (MONETARYFUNDS, TOTAL_ASSETS)** | **~1-2s** |
...
> - **EV/FCF 计算逻辑**：EV = 总市值(PE×归母净利润) + 有息负债(INTEREST_DEBT_RATIO% × 总资产) − 货币资金；FCF = FCFF_BACK(年报自由现金流)。
> - Fwd PE/PS、资产负债表、Baidu PE 三路并发请求，互不影响总耗时。
...
每只股票约需 **10-12 秒**（2次东方财富请求串行 + 盈利预测/资产负债表/Baidu PE 三路并发）。
